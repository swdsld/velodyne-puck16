...
class LiveCapture():
    def __init__(self, interface_address, queue_size=10000):
        ...
        self.packet_count = 0
        self.myqueue = Queue(maxsize=queue_size)
    ...
    def sniff_save(self, udp_port=None, packet_count=float('inf')):
        ...
        init_time = time.time()
        work = Process(target=self.sniff_continuously, args=(udp_port,))
        work.start()
        count = 0
        while count < packet_count:
            ...
            count += 1
            if count % 10000 == 0:
                print("\r{0} {1}".format(count, ' packet saved..'), end='')
    ...
    def load_packets(self):
        ...
        for packet in packets:
            if len(packet) == 0:
                continue
            if count % 10000 == 0:
                print("\r{0} {1}".format(count, ' packet loaded..'), end='')
...
def pcap2mcap(input_filename, output_filename, udp_port):
    ...
    for packet in capture._packets:
        ...
        count += 1
        if count % 1000 == 0:
            print("\r{0} {1}".format(count, ' packet converted'), end='')
    file.close()
    print("")
    return
